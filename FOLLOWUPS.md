...
| 14 | #36 | **Conflicting testimony on test scoping — verify.** The #36 agent reports plain `./gradlew test` has no tag filter in `build.gradle.kts` and would run integration specs (it scoped with `--tests "unit.*"`). Agents on #32/#33/#34/#35 ran plain `./gradlew test` and reported the `RequiresApi` specs skipped. If the #36 agent is right, earlier runs may have attempted live API calls. Relates to item 9. | open |
