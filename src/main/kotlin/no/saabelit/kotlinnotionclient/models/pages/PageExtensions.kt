 ...
 * ```
 */
inline fun <reified T : PageProperty> Page.getProperty(name: String): T? {
 ...
 */
fun Page.getNumberProperty(name: String): Double? {
    return getProperty<PageProperty.Number>(name)?.number
}

/**
 * Get a checkbox property value directly.
 */
fun Page.getCheckboxProperty(name: String): Boolean? {
    return getProperty<PageProperty.Checkbox>(name)?.checkbox
}

/**
 * Get a URL property value directly.
 */
fun Page.getUrlProperty(name: String): String? {
    return getProperty<PageProperty.Url>(name)?.url
}

/**
 * Get an email property value directly.
 */
fun Page.getEmailProperty(name: String): String? {
    return getProperty<PageProperty.Email>(name)?.email
}

/**
 * Get a phone number property value directly.
 */
fun Page.getPhoneNumberProperty(name: String): String? {
    return getProperty<PageProperty.PhoneNumber>(name)?.phoneNumber
}

/**
 * Get a select property option (returns the full SelectOption object).
 */
fun Page.getSelectProperty(name: String): SelectOption? {
    return getProperty<PageProperty.Select>(name)?.select
}

/**
 * Get a select property option name (convenience for just the name).
 */
fun Page.getSelectPropertyName(name: String): String? {
    return getSelectProperty(name)?.name
}

/**
 * Get multi-select property options (returns full SelectOption objects).
 */
fun Page.getMultiSelectProperty(name: String): List<SelectOption> {
    return getProperty<PageProperty.MultiSelect>(name)?.multiSelect ?: emptyList()
}

/**
 * Get multi-select property option names (convenience for just the names).
 */
fun Page.getMultiSelectPropertyNames(name: String): List<String> {
    return getMultiSelectProperty(name).map { it.name }
}

/**
 * Get a date property value directly.
 */
fun Page.getDateProperty(name: String): DateData? {
    return getProperty<PageProperty.Date>(name)?.date
}

/**
 * Get a people property (returns full User objects).
 */
fun Page.getPeopleProperty(name: String): List<User> {
    return getProperty<PageProperty.People>(name)?.people ?: emptyList()
}
...
fun Page.getRelationProperty(name: String): List<PageReference> {
    return getProperty<PageProperty.Relation>(name)?.relation ?: emptyList()
}
 ...
 */
fun Page.getTitleProperty(name: String): List<RichText>? {
    return getProperty<PageProperty.Title>(name)?.title
}

/**
 * Get a title property as plain text (loses formatting).
 */
fun Page.getTitleAsPlainText(name: String): String? {
    return getProperty<PageProperty.Title>(name)?.plainText
}

/**
 * Get a rich text property as full RichText objects (preserves formatting, links, etc.).
 */
fun Page.getRichTextProperty(name: String): List<RichText>? {
    return getProperty<PageProperty.RichTextProperty>(name)?.richText
}

/**
 * Get a rich text property as plain text (loses formatting).
 */
fun Page.getRichTextAsPlainText(name: String): String? {
    return getProperty<PageProperty.RichTextProperty>(name)?.plainText
}
