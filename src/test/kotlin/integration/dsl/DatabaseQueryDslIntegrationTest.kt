...
class DatabaseQueryDslIntegrationTest :
    StringSpec({
        ...
        val shouldCleanup = System.getenv("NOTION_CLEANUP_AFTER_TEST") != "false"

        lateinit var client: NotionClient
        lateinit var testDatabaseId: String

        beforeSpec {
            if (apiToken == null || testPageId == null) {
                throw Exception("NOTION_API_TOKEN and NOTION_TEST_PAGE_ID environment variables must be set")
            }
            ...
            println("Created single test database: ${testDatabase.id}")
        ...
        "Query DSL - simple filter should return matching results" {
            val pages =
                client.databases.query(testDatabaseId) {
                    filter {
